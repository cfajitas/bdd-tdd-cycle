...
describe MoviesController, type: :controller do
  describe 'Director Tests' do
    ...
    describe 'Similar' do
      ...
      it 'Redirects Home' do
        @movie1.should_receive(:director).and_return(nil)
        @movie1.should_receive(:title).and_return('Some Title')
        post :same_director, {:id => '1'}
        response.should redirect_to movies_path
      end
      it 'No Director Notice' do
        @movie1.should_receive(:director).and_return(nil)
        @movie1.should_receive(:title).and_return('Some Title')
        post :same_director, {:id => '1'}
  ...
  describe 'Completing Code Coverage' do
    ...
    describe 'Show Movie Pages' do
      before :each do
        movie = double('Movie', :title => 'Cars')
